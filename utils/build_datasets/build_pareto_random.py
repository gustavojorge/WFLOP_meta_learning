...
"""Gera datasets (random walk) para diferentes indicadores de qualidade.

Uso:
    python build_pareto_random <indicador>
...
Estrutura esperada por instância:
    ...
    meta_features/dataset/<instancia>/pareto_based/random_walk/lX_rY/

Saída:
    meta_dataset/<indicador>/pareto_based/random_walk/lX_rY.csv
"""

if len(sys.argv) != 2:
    print("Uso: python build_pareto_random <indicador>")
    sys.exit(1)
...
output_path = f'meta_dataset/{indicator_arg}/pareto_based/random_walk/'
...
for instance in os.listdir(base_path):
    ...
    def calcular_media_eps(filepath):
        """Calcula média dos valores numéricos em um arquivo (parser robusto).

        Regras:
        - Ignora linhas vazias
        - Suporta múltiplos números por linha (espaço, tab, vírgula, ponto e vírgula)
        - Converte vírgula decimal em ponto
        - Ignora tokens não numéricos
        - Emite aviso se arquivo ausente ou sem valores
        """
